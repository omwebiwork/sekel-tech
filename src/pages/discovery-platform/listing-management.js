...
import {
  listingManagementBenefits,
  listingManagementHyperlocal,
  listingManagementLeverage,
  listingManagementBanner,
} from "@/static/json/discoveryPlatform/listingManagement";
...
const ListingManagement = () => {
  const itemListSchema = useMemo(() => {
    return {
      ...
      "@type": "ItemList",
      url: "https://sekel.tech/how-it-works/discovery-platform/listing-management",
      itemListOrder: "http://schema.org/ItemListOrderAscending",
      numberOfItems: listingManagementLeverage?.cardData?.length,
      name: "Ways to Leverage Dynamic Hyperlocal Listing Management",
      description:
        "Listing Management by Sekel Tech ensures your local business info is consistent online. Automatically distribute across multiple publishers for unified listings.",
      itemListElement: listingManagementLeverage?.cardData?.map(
        (item, index) => {
          return {
            "@type": "ListItem",
            position: index + 1,
  ...
  return (
    <>
      ...
      />
      <CardSection
        {...listingManagementBenefits?.sectionData}
        ...
        renderElement={() => (
          <div className="grid grid-cols-3 gap-8">
            {listingManagementBenefits?.cardDataList?.map((item, index) => {
              return (
      ...
      />
      <CardSection
        {...listingManagementLeverage?.sectionData}
        ...
        renderElement={() => (
          <div className="grid gap-4 lg:gap-3 lg:grid-cols-3">
            {listingManagementLeverage?.cardData?.map((item, index) => {
              return (
      ...
      />
      <HyperlocalSection {...listingManagementHyperlocal} />
